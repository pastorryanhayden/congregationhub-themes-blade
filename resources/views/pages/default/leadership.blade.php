        ...
        <div class="bg-base-100">
            ...
            @if($lookingForPastor)
                <div class="mx-auto max-w-3xl px-6 py-16 lg:py-24">
                    <div class="rounded-2xl bg-info/10 border border-info/20 p-8 md:p-12">
                        <div class="flex items-start gap-4">
                            <div class="shrink-0 mt-1">
                                <svg class="size-8 text-info" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <div class="md:w-2/5 shrink-0">
                                        ...
                                        @else
                                            <div class="w-full h-64 md:h-full min-h-[300px] bg-base-300 flex items-center justify-center">
                                                <span class="text-5xl font-bold text-base-content/30">{{ getInitials($leader['name'] ?? '') }}</span>
                                            </div>
                                        @endif
                                    </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                                @foreach($section['members'] as $member)
                                    <div class="text-center">
                                        <div class="mx-auto mb-4 size-32 rounded-full overflow-hidden bg-base-300">
                                            ...
                                            @else
                                                <div class="w-full h-full flex items-center justify-center">
                                                    <span class="text-2xl font-bold text-base-content/30">{{ getInitials($member['name'] ?? '') }}</span>
                                                </div>
                                            @endif
                                        </div>
                                        ...
                                        @endif
                                        @if(!empty($member['bio']))
                                            <p class="mt-2 text-sm text-base-content/70 line-clamp-3">{{ $member['bio'] }}</p>
                                        @endif
                                    </div>
                                @endforeach
                            </div>
