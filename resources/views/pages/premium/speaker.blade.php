        ...
        <section class="bg-base-200 py-20 sm:py-28">
            <div class="max-w-4xl mx-auto px-6 lg:px-8">
                ...
                <div class="flex items-start gap-6 mb-8">
                    ...
                    @endif
                    <div>
                        <h1 class="text-3xl font-serif font-bold">{{ $name }}</h1>
                ...
                <form method="GET" class="flex flex-wrap gap-2 mb-6">
                    ...
                    @if($hasFilters)
                        <a href="" class="btn btn-ghost btn-sm rounded-full">Clear Filters</a>
                    @endif
                </form>
                ...
                @if(!empty($items))
                    <div class="space-y-3">
                        @foreach($items as $sermon)
                            <a href="{{ $sermon['url'] ?? '#' }}" class="flex items-start gap-4 group hover:bg-base-100 p-4 -mx-4 rounded-2xl transition-colors">
                ...
                @else
                    <div class="text-center py-12 text-base-content/60">
                        ...
                        @if($hasFilters)
                            <a href="" class="btn btn-ghost btn-sm mt-4 rounded-full">Clear Filters</a>
                        @endif
                    </div>
                @endif

                <div class="mt-12 pt-8 border-t border-base-300 flex gap-3">
                    <a href="/sermons/speakers" class="btn btn-ghost gap-2 rounded-full">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5 3 12m0 0 7.5-7.5M3 12h18" />
                        </svg>
                        All Speakers
                    </a>
                    <a href="/sermons" class="btn btn-ghost gap-2 rounded-full">All Sermons</a>
                </div>
            </div>
        </section>
    </main>
