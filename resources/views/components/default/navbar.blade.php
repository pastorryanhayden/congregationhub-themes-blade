...
<header class="{{ $transparent ? 'absolute inset-x-0 top-0 z-50' : 'bg-base-100' }}"
    ...
    <nav class="flex gap-x-6 justify-between items-center p-6 mx-auto max-w-7xl lg:px-8" aria-label="Global">
        ...
        <div class="hidden lg:flex lg:gap-x-12 font-sans {{ $transparent ? 'text-white' : '' }}">
                        ...
                        <div class="p-4">
                            <div class="flex relative gap-x-6 p-4 rounded-lg hover:bg-base-200 group">
                                ...
                                <div>
                                    ...
                                    <p class="mt-1 text-base-content/60">{{ __('Everything you need to know before visiting.') }}</p>
                                </div>
                            </div>
                        </div>
                                ...
                                @if($showLeadership)
                                    <li class="relative">
                                        <a href="/leadership" class="block text-sm font-semibold leading-6 text-base-content truncate">
                                            {{ __('Leadership') }}<span class="absolute inset-0"></span>
                                        </a>
                                    </li>
                                @endif
                                @if($showGospel)
            ...
            {{-- Events --}}
            @if($showEvents)
                <a href="/events" class="text-sm font-semibold leading-6 {{ $transparent ? 'text-white' : 'text-base-content' }}">{{ __('Events') }}</a>
            @endif

            {{-- Ministries --}}
            @if($showMinistries)
                <a href="/ministries" class="text-sm font-semibold leading-6 {{ $transparent ? 'text-white' : 'text-base-content' }}">{{ __('Ministries') }}</a>
            @endif
                        ...
                        <div class="p-4">
                            @if($showSermons)
                                <div class="flex relative gap-x-6 items-center p-4 text-sm leading-6 rounded-lg hover:bg-base-200 group">
                                    ...
                                    <div class="flex-auto">
                                        <a href="/sermons" class="block font-semibold text-base-content">{{ __('Sermons') }}<span class="absolute inset-0"></span></a>
                                        <p class="mt-1 text-base-content/60">{{ __('Listen to our sermons') }}</p>
                                    </div>
                                </div>
                            @endif
                            @if($hasBlogPosts)
                                <div class="flex relative gap-x-6 items-center p-4 text-sm leading-6 rounded-lg hover:bg-base-200 group">
                                    ...
                                    <div class="flex-auto">
                                        <a href="/blog" class="block font-semibold text-base-content">{{ __('Articles') }}<span class="absolute inset-0"></span></a>
                                        <p class="mt-1 text-base-content/60">{{ __('Articles from our pastor and staff') }}</p>
                                    </div>
                                </div>
                            @endif
            ...
            @else
                ...
                @if($showSermons)
                    <a href="/sermons" class="text-sm font-semibold leading-6 {{ $transparent ? 'text-white' : 'text-base-content' }}">{{ __('Sermons') }}</a>
                @endif
                @if($hasBlogPosts)
                    <a href="/blog" class="text-sm font-semibold leading-6 {{ $transparent ? 'text-white' : 'text-base-content' }}">{{ __('Articles') }}</a>
                @endif
                @foreach($resourcesPages as $page)
            ...
            <div class="flow-root mt-6">
                <div class="-my-6 divide-y divide-base-content/10">
                    <div class="py-6 space-y-2 font-sans">
                        <a href="/about" class="block py-2 px-3 -mx-3 text-base font-semibold leading-7 text-base-content rounded-lg hover:bg-base-200">{{ __('Plan Your Visit') }}</a>
                        @if($showLeadership)
                            <a href="/leadership" class="block py-2 px-8 -mx-3 text-base font-semibold leading-7 text-base-content/60 rounded-lg hover:bg-base-200">{{ __('Leadership') }}</a>
                        @endif
                        @if($showGospel)
                        ...
                        @endforeach
                        @if($showEvents)
                            <a href="/events" class="block py-2 px-3 -mx-3 text-base font-semibold leading-7 text-base-content rounded-lg hover:bg-base-200">{{ __('Events') }}</a>
                        @endif
                        @if($showMinistries)
                            <a href="/ministries" class="block py-2 px-3 -mx-3 text-base font-semibold leading-7 text-base-content rounded-lg hover:bg-base-200">{{ __('Ministries') }}</a>
                        @endif
                        @if($showSermons)
                            <a href="/sermons" class="block py-2 px-3 -mx-3 text-base font-semibold leading-7 text-base-content rounded-lg hover:bg-base-200">{{ __('Sermons') }}</a>
                        @endif
                        @if($hasBlogPosts)
                            <a href="/blog" class="block py-2 px-3 -mx-3 text-base font-semibold leading-7 text-base-content rounded-lg hover:bg-base-200">{{ __('Articles') }}</a>
                        @endif
                        @foreach($resourcesPages as $page)
